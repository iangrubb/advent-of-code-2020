  ...
  def prefix_matches(message, rules, rule_key) do
    ...
    |> Enum.map(fn option ->
      ...
      |> Enum.reduce([message], fn
        "a", remainders ->
          apply_prefix_to_remainders(remainders, "a")

        "b", remainders ->
          apply_prefix_to_remainders(remainders, "b")

        key, remainders ->
          Enum.flat_map(remainders, fn remainder -> prefix_matches(remainder, rules, key) end)
      end)
    end)
    |> Enum.concat()
    |> Enum.uniq()
